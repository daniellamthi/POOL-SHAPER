...
## AUTO-018
CATEGORY: GEOMETRY / BUILD HEALTH
SEVERITY: MEDIUM
PROBLEM: `npm run test:geometry` failed outright ("Error: metric coping texture scale") on the branch head, meaning the coping/pool-access feature work (`8167942`..`b17fd8b`, the premium coping material selector and the beige-porcelain + Pool Access follow-up) was never actually verified green after `src/lib/pool/coping-materials.ts` was introduced. Root cause: each of the 4 coping finishes declared `moduleSize` (the physical texture-repeat, metres) as 1.2-1.6 m, while `scripts/geometry-audit.ts` asserts a realistic 0.4-0.6 m slab module (matching the 0.4 m default already used for coping in `src/configurator/materials/visual-presets.ts`) — i.e. the shipped values described implausibly large ~1.5 m coping slabs and were never checked against real-world scale (the "don't tune by eye" rule `AUTO-008` already calls out).
EVIDENCE: `npm run test:geometry` throws immediately, no case count printed; confirmed the assertion (`scripts/geometry-audit.ts:1062`) and the offending values predate this session's own edits (present verbatim back through `8167942^`, before the file's history starts). Additionally, none of this feature work's new/changed files (`Skimmers.tsx`-adjacent set: `PoolFeaturesStep.tsx`, `PoolSystemStep.tsx`, `camera.ts`, `coping-materials.ts`, `materials.ts`, plus `PoolModel.tsx`, `PoolScene.tsx`, `stoneTextures.ts`, `textures.ts`, `poolConstruction.ts`, `waterDepth.ts`, `DaylightEnvironment.tsx`, and several others) had been run through `--fix`: `npm run lint` reported 470 `prettier/prettier` errors on the branch head.
EXPECTED BEHAVIOR: `test:geometry` passes; coping slab module sizes stay within the realistic 0.4-0.6 m band already established for the material category; `lint` stays at its documented baseline.
AFFECTED FILES: `src/lib/pool/coping-materials.ts` (moduleSize only: 1.3/1.2/1.5/1.6 -> 0.45/0.4/0.55/0.6, relative ordering preserved); ~24 files reformatted via `eslint --fix` (prettier only, verified no logic diff).
RISK: Low — the module-size fix only changes the coping shader's UV repeat frequency (a visual-density parameter, not geometry/placement); the lint fix is formatting-only, diffed to confirm no functional change.
STATUS: FIXED.
VERIFICATION: `npx tsc --noEmit` clean; `npm run test:geometry` — "Construction audit passed: ... four coping finishes." / "Geometry audit passed: 108 shape/dimension/system cases, 6 custom-shape offset cases, 12 guardrail regressions, 72 camera poses and 24 clamped drag steps."; `npm run lint` back to baseline (0 errors, pre-existing `react-refresh` warnings only); `npm run build` clean.
