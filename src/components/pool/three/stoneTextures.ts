...
export type StoneMaps = { colorMap: THREE.DataTexture; normalMap: THREE.DataTexture; roughnessMap: THREE.DataTexture };
...
function memoizedTemplate(cache: Map<number, StoneMaps>, size: number, build: () => StoneMaps): StoneMaps {
...
export function createTravertineMaps(size = 512): StoneMaps {
  return memoizedTemplate(travertineCache, size, () => {
    ...
    for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
      ...
      const sediment = valueNoise(u, v, 5) * 0.5 + valueNoise(u, v, 19) * 0.3 + valueNoise(u, v, 47) * 0.2;
      ...
      const band = Math.sin(u * 26 + valueNoise(u, v, 3) * 3.5) * 0.06
      ...
      const pore = seed > 0.4 ? 1 - THREE.MathUtils.smoothstep(Math.hypot(dx, dy), 0.025, 0.08 + seed * 0.12) : 0;
    ...
    return { colorMap: makeTexture(color, size, true), normalMap: makeTexture(normal, size, false), roughnessMap: makeTexture(rough, size, false) };
  });
}
...
export function createLimestoneMaps(size = 512): StoneMaps {
  return memoizedTemplate(limestoneCache, size, () => {
    ...
    for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
      ...
      const cloud = valueNoise(u, v, 4) * 0.55 + valueNoise(u, v, 11) * 0.3 + valueNoise(u, v, 29) * 0.15;
...
export function createPrunMaps(size = 512): StoneMaps {
  return memoizedTemplate(prunCache, size, () => {
    ...
    for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
      const u = x / size, v = y / size;
      const sediment = valueNoise(u, v, 7) * 0.45 + valueNoise(u, v, 23) * 0.35 + valueNoise(u, v, 59) * 0.2;
      ...
      const pore = seed > 0.55 ? 1 - THREE.MathUtils.smoothstep(Math.hypot(dx, dy), 0.02, 0.055 + seed * 0.05) : 0;
